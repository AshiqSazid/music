# Music Intelligence Assistant - Streamlit App with YAMNet + OpenL3 + MP3 Support
...
- \U0001F680 Virality prediction from audio features
- \U0001F4AC Local LLM response via Ollama (Mistral)
""")
...
if uploaded_file:
    ...
    # === Instrument Detection with YAMNet ===
    ...
    if proportions:
        st.write("### Instrument Breakdown (% of total energy)")
        st.bar_chart(pd.Series(proportions))
    else:
        st.warning("No recognizable instruments detected.")

    # === Audio Mood Embedding with OpenL3 ===
    st.subheader("2. Audio Mood Embedding (OpenL3)")
    ...
    st.success(f"OpenL3 audio embedding shape: {mean_embedding.shape}")

    # === Virality Prediction ===
    st.subheader("3. Virality Prediction")
    ...
    train_y = np.clip((0.3 * train_X["tempo"] +
    ...
    # === LLM Prompt & Response ===
    st.subheader("4. LLM Feedback via Mistral")
    prompt_data = {
...
Questions:
1. Would this song fit a workout, chill, romantic, or viral TikTok playlist? Why?
2. What production or lyrical changes could boost impact?
3. Recommend marketing strategies and platforms.
"""

    st.code(deepseek_prompt, language="markdown")

    if st.button("Get AI Feedback (Ollama - Mistral)"):
        with st.spinner("Querying Mistral locally via Ollama..."):
            response = query_ollama_mistral(deepseek_prompt)
        st.markdown("### \U0001F4AC Mistral's Feedback")
        st.markdown(response)
else:
    st.info("Please upload a .wav or .mp3 audio file to begin.")
